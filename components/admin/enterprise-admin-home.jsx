...
export default function EnterpriseAdminHomePage() {
  ...
  const commandSections = useMemo(() => {
    const nav = meta?.nav || [];
    const order = meta?.groupOrder || [];
    const labelsAr = meta?.navGroupsAr || {};
    const labels = meta?.navGroups || {};
    return order
      .map((id) => ({
        id,
        label: labelsAr[id] || labels[id] || id,
        items: nav.filter((n) => n.group === id && n.id !== "dashboard"),
      }))
      ...
      >
        <div>
          ...
          <p style={{ margin: "4px 0 0", color: "#6b7280", fontSize: 13 }}>
            Live data from enterprise store + curriculum registries. No static
            KPIs.
        ...
        <button
          type="button"
          onClick={() => load()}
          style={{ padding: "0.45rem 0.8rem", borderRadius: 8 }}
        >
          Refresh
        </button>
      </div>
