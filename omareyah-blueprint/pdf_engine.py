...
def build_pdf(charts: dict[str, Path]) -> Path:
    ...
    section(
        ...
        [
            ...
            "الخطة تتضمن مساراً مستقبلياً لفتح فروع جديدة بعد إثبات قابلية النموذج للنسخ — لا توسعاً مبكراً بلا جاهزية.",
        ],
        table=[
            ["البُعد", "قيمة التعيين المقترح"],
            [["الجودة", "إطار قابل للقياس"], ["القبول", "رسالة مبنية على تجربة"], ["الهوية", "تموضع دولي أوضح"], ["الاستدامة", "منظومة لا فرد فقط"], ["الفروع مستقبلاً", "توسع مشروط ببوابات قرار"]],
            [250, 250],
        ],
    )
    ...
    section(
        "11",
        "السنوات 2–5 وفتح فروع جديدة مستقبلاً",
        [
            "تميز أكاديمي، تموضع علامة، استدامة مالية، ونضج رقمي — مع نضج فصل الهوية كجزء من التموضع.",
            "مسار استراتيجي صريح: فتح فروع جديدة مستقبلاً بعد أن يصبح النموذج الأم قابلاً للنسخ، لا قبل ذلك.",
            "السنة 2: إشغال وتوثيق النموذج. السنة 3: دراسة مواقع وجدوى. السنة 4: فرع أول منضبط. السنة 5: تثبيت وتقييم فرع ثانٍ عند الجاهزية فقط.",
        ],
        chart_key="roadmap",
        table=[
            ["بوابة قبل أي فرع", "المعنى"],
            [
                ["جودة مستقرة", "الموقع الأم لا يُدار بالأزمات"],
                ["هوية قابلة للنقل", "فصل إنترناشونال واضح"],
                ["جدوى تحفظية", "سيناريو مالي واقعي"],
                ["قيادة جاهزة", "لا اعتماد على فرد واحد"],
                ["موافقة المجلس", "موقع + موازنة + مؤشرات"],
            ],
            [200, 300],
        ],
    )
    ...
    section(
        ...
        [
            ...
            "وعلى المدى المتوسط: جاهزية مؤسسية لفتح فروع جديدة بقرار مجلس مبني على بيانات لا على حماس توسعي.",
        ],
        table=[
            ...
            [
                ...
                ["الفروع مستقبلاً", "ملف نموذج قابل للنسخ"],
            ],
            [220, 280],
        ],
    )
    ...
    section(
        ...
        table=[
            ...
            [
                ...
                ["فرع قبل الجاهزية", "بوابات قرار + موافقة مجلس"],
            ],
            [240, 260],
        ],
    )
    ...
    section(
        ...
        [
            ...
            "هذا القرار يبني أنظمة وهوية أوضح للإنترناشونال منفصلة تقريباً عن المسار الوطني، ويمهّد لتوسع مستقبلي بفروع جديدة بعد الجاهزية — لا مجرد إدارة يومية إضافية.",
        ],
        bullets=[
            "اعتماد الدور والصلاحيات.",
            "اعتماد خطة المائة يوم ومسار فصل الهوية.",
            "اعتماد سياسة: العمريّة أولاً، والشريك للتنفيذ فقط.",
            "اعتماد مسار فتح الفروع مستقبلاً عبر بوابات قرار المجلس.",
