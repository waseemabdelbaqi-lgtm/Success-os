...
def create_charts() -> dict[str, Path]:
    ...
    tracks = {
        ...
        "جاهزية فتح فروع": [35, 55, 72, 88],
        "نضج رقمي": [55, 68, 80, 88],
    }
    colors = [f"#{BURGUNDY_HEX}", f"#{GOLD_HEX}", "#666666", "#2F5D50", "#8A6A4A"]
    for (name, vals), c in zip(tracks.items(), colors):
        ax.plot(years, vals, marker="o", linewidth=2.2, color=c, label=name)
    ax.set_ylim(20, 100)
    ax.set_title("خارطة الطريق الاستراتيجية — السنوات 2 إلى 5 (بما فيها جاهزية الفروع)", fontproperties=fp, fontsize=12, color=f"#{BURGUNDY_HEX}")
    ax.legend(prop=fp, ncol=3, frameon=False, loc="lower right", fontsize=8)
    ...
    # 3b) Branch expansion phases
    fig, ax = plt.subplots(figsize=(10.2, 2.9), dpi=180)
    fig.patch.set_facecolor("white")
    phases = [
        (0, 1.2, "سنة 2\nإشغال + نموذج قابل للنسخ", f"#{GOLD_HEX}"),
        (1.2, 1.2, "سنة 3\nدراسة مواقع وتجهيز", f"#{BURGUNDY_HEX}"),
        (2.4, 1.3, "سنة 4\nإطلاق فرع أول منضبط", "#2F5D50"),
        (3.7, 1.3, "سنة 5\nتثبيت/تقييم فرع ثانٍ", "#666666"),
    ]
    y = 0.4
    for start, width, label, color in phases:
        ax.barh([y], [width], left=start, height=0.38, color=color, edgecolor="white")
        ax.text(start + width / 2, y, label, ha="center", va="center", color="white", fontproperties=fp, fontsize=9)
    ax.set_xlim(0, 5)
    ax.set_ylim(0, 1)
    ax.axis("off")
    ax.set_title("مسار فتح الفروع الجديدة مستقبلاً — مشروط بجاهزية الجودة والاستدامة", fontproperties=fp, fontsize=12, color=f"#{BURGUNDY_HEX}", pad=8)
    fig.tight_layout()
    paths["branches"] = ASSETS / "chart_branch_expansion.png"
    fig.savefig(paths["branches"], bbox_inches="tight", facecolor="white")
    plt.close(fig)
...
def build_docx(charts: dict[str, Path], watermark: Path) -> Path:
    ...
    add_table(
        ...
        [
            ...
            ["9–11. خطط التنفيذ", "100 يوم · سنة · 2–5 سنوات · فتح فروع مستقبلاً"],
    ...
    add_ar(
        ...
        "كما تضع الخطة مساراً مستقبلياً لفتح فروع جديدة بعد إثبات قابلية النموذج للنسخ في الموقع الأم.",
    )
    ...
    add_table(
        ...
        [
            ...
            ["التوسع بالفروع", "مسار مشروط بجاهزية الجودة والنسخ المؤسسي", "توسع مبكر بلا نموذج جاهز"],
        ],
    )
    add_callout(
        doc,
    ...
    add_table(
        ...
        [
            ...
            ["فتح فروع جديدة", "رفع إشغال النموذج الأم وجعله قابلاً للنسخ", "دراسات مواقع + دراسة جدوى أولية", "إطلاق فرع أول منضبط ثم تقييم فرع ثانٍ"],
        ],
    )

    add_subtitle(doc, "11.1 فتح فروع جديدة مستقبلاً (محور استراتيجي)")
    add_ar(
        doc,
        "تتضمن الخطة الاستراتيجية إمكانية فتح فروع جديدة لقسم/برنامج الإنترناشونال مستقبلاً، لكن ليس كهدف مستقل عن الجودة. "
    ...
    add_table(
        ...
        [
            ["السنة 2", "تعظيم إشغال الموقع الحالي + توثيق نموذج التشغيل (أدلة إجراءات، جودة، هوية، قبول)", "استقرار مؤشرات الجودة/الاستبقاء + وضوح هوية الإنترناشونال", "ملف «نموذج قابل للنسخ»"],
            ["السنة 3", "مسح مواقع محتملة، طلب سوق، منافسة، تكاليف تأسيسية وتشغيلية", "دراسة جدوى أولية مع سيناريوهات تحفظية", "مذكرة مواقع قصيرة + توصية مبدئية"],
            ["السنة 4", "إطلاق فرع أول بحجم منضبط ونفس معايير الهوية والجودة", "تحقق حد أدنى من الإشغال والجودة خلال عام التشغيل الأول", "تقرير إطلاق + دروس مستفادة"],
            ["السنة 5", "تثبيت الفرع الأول وتقييم جاهزية فرع ثانٍ فقط إذا نجحت المعايير", "عدم تكرار أخطاء الفرع الأول؛ قدرة قيادية كافية", "قرار توسع/تأجيل مبني على بيانات"],
        ],
    )
    add_callout(
        doc,
        "بوابات قرار قبل فتح أي فرع",
        [
            "جودة أكاديمية وتشغيلية مستقرة في الموقع الأم (لا تُدار الأزمات يومياً).",
            "هوية بصرية وتشغيلية واضحة للإنترناشونال قابلة للنقل دون خلط مع المسار الوطني.",
            "دراسة جدوى مالية بسيناريو تحفظي (إيراد/تكلفة/فترة استرداد معقولة).",
            "فريق قيادة وطبقة وسطى جاهزة للنسخ (لا يعتمد الفرع على شخص واحد فقط).",
            "موافقة المجلس على الموقع، الموازنة، والجدول، ومؤشرات السنة الأولى للفرع.",
        ],
    )
    add_subtitle(doc, "ما الذي يُنسخ إلى الفرع الجديد؟")
    for t in [
        "إطار الجودة والملاحظة الصفية وتطوير المعلمين.",
        "رحلة القبول والتواصل الأسري وقوالب الهوية الدولية.",
        "لوحة مؤشرات أساسية (تحويل، استبقاء، جودة، رضا تواصل).",
        "حوكمة التقارير الأسبوعية/الربعية وربط الفرع بمدير البرامج الدولية.",
        "حزم الدعم الأكاديمي والإرشاد والمسارات الرقمية الحدّ الأدنى.",
    ]:
        add_bullet(doc, t)
    ...
    add_table(
        ...
        [
            ...
            ["جاهزية التوسع بالفروع", "توثيق نموذج تشغيل قابل للنسخ", "قرار مدروس لفرع أول عند تحقق البوابات"],
        ],
    )
    ...
    add_table(
        ...
        [
            ...
            ["فتح فرع قبل جاهزية النموذج الأم", "تكرار ضعف الجودة وتشتيت القيادة", "متوسط", "بوابات قرار إلزامية + موافقة مجلس قبل أي إطلاق"],
        ],
    )
    ...
    for t in [
        ...
        "لأنه يُخضِع التوسع وفتح الفروع في السنوات 2–5 لنضج الجودة والاستدامة وبوابات قرار المجلس، لا لضغط النمو فقط.",
    ]:
        add_bullet(doc, t)
    ...
    add_callout(
        ...
        [
            ...
            "اعتماد مبدأ التوسع المستقبلي بفروع جديدة بعد تحقق بوابات الجاهزية ودراسة الجدوى.",
