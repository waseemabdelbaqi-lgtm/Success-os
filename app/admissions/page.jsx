...
export default function AdmissionsPage(){
  ...
  const results=useMemo(()=>globalInstitutions.filter(x=>(studyCountry==='الكل'||x.country===studyCountry)&&(city==='الكل'||x.city===city)&&(field==='الكل'||x.fields.includes(field))&&(mode==='الكل'||x.modes.includes(mode))&&x.systems.includes(system)&&(!query||`${x.name} ${x.country} ${x.city} ${x.fields.join(' ')}`.toLowerCase().includes(query.toLowerCase()))),[studyCountry,city,field,mode,system,query]);
  ...
  return <div className="os-page"><InnerNav active="admissions"/><main className="os-page-content admission-match-page">
    ...
    <section className="student-admission-profile"><header><div><small>01 — ملف الطالب</small><h2>من أنت وما الشهادة التي تحملها؟</h2></div><b>النتائج تتغير فوراً</b></header><div>
      ...
      <label>الدرجة المطلوبة<select><option>بكالوريوس</option><option>دبلوم</option><option>ماجستير</option><option>دكتوراه</option></select></label>
