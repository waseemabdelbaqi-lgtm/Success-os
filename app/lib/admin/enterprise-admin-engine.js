...
export function listModuleItems(moduleId, options = {}) {
  ...
  if (moduleId === 'recorded-lessons') {
    const catalogSort =
      options.catalogSort ||
      (['newest', 'popularity', 'rating', 'price_asc', 'price_desc', 'duration', 'last_updated'].includes(
        options.sort,
      )
        ? options.sort
        : 'newest');
    const catalog = applyRecordedLessonCatalog(items, {
      ...
      educationalSystem: options.educationalSystem,
      curriculum: options.curriculum,
      qualification: options.qualification,
      grade: options.grade,
      subjectFamily: options.subjectFamily,
    ...
    if (options.lessonSource) {
      ...
      if (sourceFilter !== 'ALL') {
        items = items.filter((row) =>
          matchesLessonSourceFilter(row.lessonSource || row.source_type, sourceFilter),
        );
      }
    }
  ...
  if (moduleId === 'recorded-lessons' && options.status) {
    items = items.filter(
      (row) => row.status === options.status || row.publication_status === options.status,
    );
  }
  ...
  if (typeof schema.formatRow === 'function') {
    ...
    pageItems = pageItems.map((row) => ({
      ...
      lessonSourceLabel: lessonSourceLabel(row.lessonSource || row.source_type),
    }));
  }
...
export function mutateModule(moduleId, action, payload = {}) {
  ...
  if (action === 'add' || action === 'create') {
    ...
    if (moduleId === 'recorded-lessons') {
      ...
      const source = normalizeLessonSource(payload.lessonSource || payload.source_type);
      ...
      row.publishedAt = payload.publishedAt || (row.status === 'published' || row.status === 'PUBLISHED' ? nowIso() : null);
    }
  ...
  if (action === 'edit' || action === 'update') {
    ...
    if (moduleId === 'recorded-lessons') {
      ...
      if (payload.lessonSource != null || payload.source_type != null) {
        const source = normalizeLessonSource(payload.lessonSource || payload.source_type);
        if (!source || source === 'ALL') {
          return { ok: false, error: 'LESSON_SOURCE_REQUIRED' };
        }
      ...
      for (const key of ['country', 'curriculum', 'grade', 'subject', 'teacherName']) {
