...
export default function StudentService(){
 ...
 return <div className="os-page phase11-legacy-page"><InnerNav active="access"/><main className="os-page-content market-page service-detail-page"><header><small>{offer.providerType}</small><h1>{offer.provider}</h1><p>{offer.subject} — {offer.service}</p></header><section className="service-facts"><article><b>المادة</b><span>{offer.subject}</span></article><article><b>مدة المنتج</b><span>{offer.duration}</span></article><article><b>موعد/حالة التفعيل</b><span>{offer.schedule}</span></article><article><b>السعر النهائي</b><span>{rewardApplied?'30 نقطة':`${offer.currency} ${price.toFixed(2)}`}</span></article>{offer.rewardPoints>0&&!rewardApplied&&<article><b>نقاط الولاء</b><span>+{offer.rewardPoints} نقطة بعد التفعيل</span></article>}</section><aside className="service-commission"><p>{rewardApplied?'سيتم خصم 30 نقطة عند تفعيل المادة، وليس عند مغادرة الصفحة.':`السعر يتضمن عمولة المنصة 10% (${offer.currency} ${fee.toFixed(2)}) ولا تُضاف على الطالب.`}</p></aside><footer className="service-actions"><button onClick={()=>router.back()}>رجوع إلى النتائج</button><button className="primary" onClick={checkout}>متابعة إلى الشراء</button></footer></main></div>
}
