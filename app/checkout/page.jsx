...
const emptyOrder={item:'',service:'',subject:'',duration:'',schedule:'',base:0,fee:0,total:0,offerId:'',kind:'',months:0,points:0,reward:'',country:'',system:'',stage:'',grade:'',university:'',major:'',academicYear:'',inquiryId:'',nationality:'',applicantType:''};
export default function Checkout(){
 ...
 return <div className="os-page phase11-legacy-page"><InnerNav active="access"/><main className="os-page-content checkout-page">
  <header><small>SUCCESS OS SECURE CHECKOUT</small><h1>{isInquiry?`دفع رسوم التواصل $${UNIVERSITY_INQUIRY_FEE_USD}`:order.reward?'تفعيل المادة بالنقاط':'إتمام الشراء'}</h1><p>{isInquiry?'بعد نجاح الدفع تُفتح قناة الإشعارات إن كانت الجامعة مشتركة، أو مسودة الإيميل الرسمي إن لم تكن مشتركة.':'بعد نجاح العملية تُفعّل المادة المسجلة فورًا داخل لوحة الطالب.'}</p><button className="owner-preview-purchase" disabled={busy} onClick={ownerPreview}>{busy?'جارٍ التفعيل…':isInquiry?'اعتبرني دفعت — افتح قناة التواصل':'اعتبرني اشتريت — انتقل للوحة الطالب'}</button></header>
